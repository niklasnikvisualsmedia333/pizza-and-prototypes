...
const augustEvent = eventsSource.match(/\{\n    slug: 'ai-in-software-development-2026',[\s\S]*?\n  \},/)?.[0] ?? '';
assert(augustEvent, 'August event is missing');
assert(augustEvent.includes("status: 'past'"), 'August event is not marked as past');
assert(augustEvent.includes("registrationStatus: 'closed'"), 'August event registration is not closed');
assert(!augustEvent.includes('registrationUrl'), 'Past August event still has an active registration URL');
assert(augustEvent.includes("speaker: 'Sebastian Klietsch'"), 'Completed August event speaker is missing');
assert(augustEvent.includes("startTime: '18:00'"), 'August event start time is missing');
...
assert(bundleText.includes('Sebastian Klietsch'), 'Completed August event speaker is missing from the built bundle');
assert(!bundleText.includes('forms.gle/pNbme2mcW9WrHNDG6'), 'Past August event registration URL leaked into the built bundle');
